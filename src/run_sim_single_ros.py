...
class SimpleClient(SDClient):
    ...
    def on_msg_recv(self, json_packet):
        ...
        if json_packet['msg_type'] == "telemetry":
            self.time_stamp = time.time()  # for time sensitive algos
            self.pose[:3] = np.array([json_packet["pos_x"] - 50, json_packet["pos_y"] - 50, json_packet["pos_z"]])
            self.pose[3:] = np.array([json_packet["quat_x"], json_packet["quat_y"],
                                     json_packet["quat_z"], json_packet["quat_w"]])
            roll, pitch, yaw = tf.transformations.euler_from_quaternion((self.pose[3], self.pose[4],
                                                                        self.pose[5], self.pose[6]))
...
class RacecarState:
    ...
    def __init__(self):
        ...
        self.state_publisher = rospy.Publisher("car_pose",
                                               PoseStamped, queue_size=1)
        self.odom_publisher = rospy.Publisher("car_odom",
                                              Odometry, queue_size=10)
        # Publishes joint values
        self.cur_joints_pub = rospy.Publisher("joint_states",
                                              JointState, queue_size=1)

        self.imu_publisher = rospy.Publisher("imu",
                                             Imu, queue_size=10)
        ...
        self.control_sub = rospy.Subscriber("mux/ackermann_cmd_mux/output",
    ...
    def create_client(self):
        ...
        # Start Clients
        self.client = SimpleClient(address=(host, port),
                                   poll_socket_sleep_time=0.001)
        msg = '{"msg_type" : "load_scene", "scene_name" : "generated_road" }'
        self.client.send(msg)
        time.sleep(1)  # Wait briefly for the scene to load.
        # initialization. The position will be set to 50,50 when the car loads
    ...
    def input_callback(self, data):
        ...
        steering = - data.drive.steering_angle * 57.3 / 25  # scaling
    ...
    def init_pose_cb(self, data):
        ...
        msg = '{ "msg_type" : "car_config",' \
            ...
            + ' "font_size" : "100",' \
            + ' "start_X" : "' \
            + str(round(X, 3)) + '", "start_Y" : "' + str(round(Y, 3)) + '", "start_Z" : "0.20",' \
            + ' "yaw" : "' + str(round(yaw - m.pi / 2, 3)) + '" }'
        self.client.send(msg)
        time.sleep(1)

    def timer_cb(self, event):
        """
        timer callback: called at regular intervals to update state data
        ...
        try:
            ...
            if np.abs(delta) < 1e-2:
                ...
                joint_outer_throttle = (((self.CAR_WIDTH + h_val) / (0.5 * self.CAR_WIDTH + h_val)) * self.client.speed
                                        * (1.0 / self.UPDATE_RATE) / self.CAR_WHEEL_RADIUS)
                joint_inner_throttle = (((h_val) / (0.5 * self.CAR_WIDTH + h_val)) * self.client.speed
                                        * (1.0 / self.UPDATE_RATE) / self.CAR_WHEEL_RADIUS)
                joint_outer_steer = m.atan2(self.CAR_LENGTH,
